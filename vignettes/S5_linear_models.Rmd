...
ggplot(ndf |> filter(oncosign %in% c("TP53/NF1", "TERT/ZNRF3")), 
    aes(y=logEGR1, x=factor(oncosign))) + geom_boxplot()
tpvtert = ndf |> filter(oncosign %in% c("TP53/NF1", "TERT/ZNRF3"))
t.test(logEGR1~oncosign, data=tpvtert, var.equal=TRUE)
t.test(logEGR1~oncosign, data=tpvtert, var.equal=FALSE) # more general

ndf |> filter(oncosign %in% c("TP53/NF1", "TERT/ZNRF3")) |> 
         lm(logEGR1~oncosign, data=_) |> summary()
```
